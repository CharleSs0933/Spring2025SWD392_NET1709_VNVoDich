  ...
  return (
    <div className="max-w-7xl mx-auto p-6 bg-white shadow-lg rounded-xl space-y-6">
        ...
        <div className="text-start flex flex-col top-[30%] space-y-2 absolute mx-2  inset-0 z-20">
          <div className="my-4">
            ...
            <div className="flex gap-2   items-center">
              ...
              />
            <p className="text-orange-400 font-semibold text-lg hover:underline cursor-pointer">{course?.tutor?.profile?.full_name}</p>
      ...
      <div className="flex gap-5 border w-[80%] p-10 bg-white-100 text-black">
        <div className="underline cursor-pointer px-3 text-lg font-semibold">{course?.lessons?.length} Courses series</div>
        <div className="border-l px-3 text-lg font-semibold flex items-center gap-2">{course?.grade} <Star size={15} color="blue"/></div>
        <div className="border-l px-3 text-lg font-semibold">Beginner level</div>
        <div className="border-l px-3 text-lg font-semibold">1 month</div>
        <div className="border-l px-3 text-lg font-semibold">Flexible schedule</div>
      </div>
          ...
          {course?.lessons?.map((lesson: any) => {
            ...
            return (
              <div key={lesson.id} className="bg-white p-4 rounded-md shadow">
                ...
                <p className={`text-sm text-gray-600 mt-2 ${isExpanded ? "block" : "line-clamp-2"}`}>
                  {lesson.description}
                </p>

                {isExpanded && (
                  <div className="mt-2 text-sm text-gray-700 space-y-2">
                    <p><strong>🎯 Learning Objectives:</strong> {lesson.learning_objectives || "Not provided"}</p>
                    <p><strong>📌 Materials Needed:</strong> {lesson.materials_needed || "Not required"}</p>
                  </div>
                )}
              </div>
            );
